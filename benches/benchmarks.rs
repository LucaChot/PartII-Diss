...
pub fn mult_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("Hash Mult", |b| b.iter(|| {
      ...
      matmul.parallel_mult::<Hash>(black_box(a),black_box(b))
    }));
    c.bench_function("FoxOtto Mult", |b| b.iter(|| {
      ...
      let mut processor = Processor::new(PROCESSOR_HEIGHT, PROCESSOR_WIDTH, Box::new(TaurusNetworkBuilder::new()));
      let mut matmul : MatMul<isize> = MatMul::new(&mut processor);
      matmul.parallel_mult::<FoxOtto>(black_box(a),black_box(b))
    }));
    c.bench_function("Cannon Mult", |b| b.iter(|| {
      ...
      let mut processor = Processor::new(PROCESSOR_HEIGHT, PROCESSOR_WIDTH, Box::new(TaurusNetworkBuilder::new()));
      let mut matmul : MatMul<isize> = MatMul::new(&mut processor);
      matmul.parallel_mult::<Cannon>(black_box(a),black_box(b))
    }));
}
...
pub fn square_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("Hash Square", |b| b.iter(|| {
      ...
      let mut processor = Processor::new(PROCESSOR_HEIGHT, PROCESSOR_WIDTH, Box::new(TaurusNetworkBuilder::new()));
      let mut matmul : MatMul<isize> = MatMul::new(&mut processor);
      matmul.parallel_square::<Hash>(black_box(a),black_box(iterations))
    }));
    c.bench_function("FoxOtto Square", |b| b.iter(|| {
      ...
      let mut processor = Processor::new(PROCESSOR_HEIGHT, PROCESSOR_WIDTH, Box::new(TaurusNetworkBuilder::new()));
      let mut matmul : MatMul<isize> = MatMul::new(&mut processor);
      matmul.parallel_square::<FoxOtto>(black_box(a),black_box(iterations))
    }));
    c.bench_function("Cannon Square", |b| b.iter(|| {
      ...
      let mut processor = Processor::new(PROCESSOR_HEIGHT, PROCESSOR_WIDTH, Box::new(TaurusNetworkBuilder::new()));
      let mut matmul : MatMul<isize> = MatMul::new(&mut processor);
      matmul.parallel_square::<Cannon>(black_box(a),black_box(iterations))
    }));
}
  ...
  for processor_side in 3..11 {
    group.bench_with_input(BenchmarkId::new("HASH", processor_side), &processor_side, 
      |b, processor_side| b.iter(|| {
        let a = vec![vec![2; MATRIX_WIDTH]; MATRIX_HEIGHT];
        let iterations = f64::ceil(f64::log2(a.len() as f64)) as usize;
        let mut processor = Processor::new(*processor_side, *processor_side, Box::new(TaurusNetworkBuilder::new()));
        let mut matmul : MatMul<isize> = MatMul::new(&mut processor);
        matmul.parallel_square::<Hash>(black_box(a),black_box(iterations))
      }));
